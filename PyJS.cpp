...
void CScriptVar::init() {
    ...
    data = PYJS_BLANK_DATA;
    intData = 0;
    doubleData = 0;
}

CScriptVar *CScriptVar::getReturnVar() {
    return getParameter(PYJS_RETURN_VAR);
}

void CScriptVar::setReturnVar(CScriptVar *var) {
    findChildOrCreate(PYJS_RETURN_VAR)->replaceWith(var);
}
...
CScriptVarLink *CScriptVar::findChildOrCreate(const string &childName, int varFlags) {
    ...
    return addChild(childName, new CScriptVar(PYJS_BLANK_DATA, varFlags));
}
...
CScriptVar *CScriptVar::getArrayIndex(int idx) {
    ...
    else return new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_NULL); // undefined
}
...
void CScriptVar::copyValue(CScriptVar *val) {
    if (val) {
      ...
      while (child) {
        ...
        if (child->name != PYJS_PROTOTYPE_CLASS)
...
CPyJS::CPyJS() {
    l = 0;
    root = (new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT))->ref();
    // Add built-in classes
    stringClass = (new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT))->ref();
    arrayClass = (new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT))->ref();
    objectClass = (new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT))->ref();
...
void CPyJS::execute(const string &code) {
    CScriptLex *oldLex = l;
    vector<CScriptVar*> oldScopes = scopes;
    l = new CScriptLex(code);
#ifdef PYJS_CALL_STACK
    call_stack.clear();
#endif
    scopes.clear();
...
CScriptVarLink CPyJS::evaluateComplex(const string &code) {
    CScriptLex *oldLex = l;
    vector<CScriptVar*> oldScopes = scopes;
...
string CPyJS::evaluate(const string &code) {
    return evaluateComplex(code).var->getString();
}

void CPyJS::parseFunctionArguments(CScriptVar *funcVar) {
  l->match('(');
  while (l->tk!=')') {
...
void CPyJS::addNative(const string &funcDesc, JSCallback ptr, void *userdata) {
    CScriptLex *oldLex = l;
    ...
    while (l->tk == '.') {
      ...
      if (!link) link = base->addChild(funcName, new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT));
      base = link->var;
      funcName = l->tkStr;
      l->match(LEX_ID);
    }

    CScriptVar *funcVar = new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_FUNCTION | SCRIPTVAR_NATIVE);
...
CScriptVarLink *CPyJS::parseFunctionDefinition() {
  // actually parse a function...
  l->match(LEX_R_FUNCTION);
  string funcName = PYJS_TEMP_NAME;
  /* we can have functions without names */
  if (l->tk==LEX_ID) {
    funcName = l->tkStr;
    l->match(LEX_ID);
  }
  CScriptVarLink *funcVar = new CScriptVarLink(new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_FUNCTION), funcName);
 ...
 */
CScriptVarLink *CPyJS::functionCall(bool &execute, CScriptVarLink *function, CScriptVar *parent) {
  if (execute) {
    ...
    CScriptVar *functionRoot = new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_FUNCTION);
    if (parent)
    ...
    CScriptVarLink *returnVarLink = functionRoot->addChild(PYJS_RETURN_VAR);
...
CScriptVarLink *CPyJS::factor(bool &execute) {
    if (l->tk=='(') {
        l->match('(');
    ...
    if (l->tk==LEX_R_NULL) {
        l->match(LEX_R_NULL);
        return new CScriptVarLink(new CScriptVar(PYJS_BLANK_DATA,SCRIPTVAR_NULL));
    }
    if (l->tk==LEX_R_UNDEFINED) {
        l->match(LEX_R_UNDEFINED);
        return new CScriptVarLink(new CScriptVar(PYJS_BLANK_DATA,SCRIPTVAR_UNDEFINED));
    }
    if (l->tk==LEX_ID) {
    ...
    if (l->tk=='{') {
        CScriptVar *contents = new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT);
    ...
    if (l->tk=='[') {
        CScriptVar *contents = new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_ARRAY);
        /* JSON-style array */
        l->match('[');
        int idx = 0;
        while (l->tk != ']') {
    ...
    if (l->tk==LEX_R_FUNCTION) {
      CScriptVarLink *funcVar = parseFunctionDefinition();
        if (funcVar->name != PYJS_TEMP_NAME)
    ...
    if (l->tk==LEX_R_NEW) {
      ...
      if (execute) {
        ...
        l->match(LEX_ID);
        CScriptVar *obj = new CScriptVar(PYJS_BLANK_DATA, SCRIPTVAR_OBJECT);
        ...
        if (objClassOrFunc->var->isFunction()) {
          ...
          obj->addChild(PYJS_PROTOTYPE_CLASS, objClassOrFunc->var);
          if (l->tk == '(') {
            l->match('(');
            l->match(')');
          }
...
CScriptVarLink *CPyJS::unary(bool &execute) {
    CScriptVarLink *a;
    if (l->tk=='!') {
        l->match('!'); // binary not
        a = factor(execute);
...
CScriptVarLink *CPyJS::term(bool &execute) {
...
CScriptVarLink *CPyJS::expression(bool &execute) {
    bool negate = false;
    if (l->tk=='-') {
        l->match('-');
        negate = true;
...
CScriptVarLink *CPyJS::shift(bool &execute) {
...
CScriptVarLink *CPyJS::condition(bool &execute) {
...
CScriptVarLink *CPyJS::logic(bool &execute) {
...
CScriptVarLink *CPyJS::ternary(bool &execute) {
...
CScriptVarLink *CPyJS::base(bool &execute) {
...
void CPyJS::statement(bool &execute) {
    if (l->tk==LEX_ID ||
        l->tk==LEX_INT ||
        l->tk==LEX_FLOAT ||
        l->tk==LEX_STR ||
        ...
        CScriptLex *oldLex = l;
        int loopCount = PYJS_LOOP_MAX_ITERATIONS;
        ...
        if (loopCount<=0) {
            root->trace();
            TRACE("WHILE Loop exceeded %d iterations at %s\n", PYJS_LOOP_MAX_ITERATIONS, l->getPosition().c_str());
        ...
        if (l->tk != ';')
          result = base(execute);
        if (execute) {
          CScriptVarLink *resultVar = scopes.back()->findChild(PYJS_RETURN_VAR);
          if (resultVar)
...
CScriptVar *CPyJS::getScriptVariable(const string &path) {
    // traverse path
    size_t prevIdx = 0;
...
const string *CPyJS::getVariable(const string &path) {
...
bool CPyJS::setVariable(const std::string &path, const std::string &varData) {
    CScriptVar *var = getScriptVariable(path);
    // return result
    if (var) {
        if (var->isInt())
...
CScriptVarLink *CPyJS::findInScopes(const std::string &childName) {
...
CScriptVarLink *CPyJS::findInParentClasses(CScriptVar *object, const std::string &name) {
    // Look for links to actual parent classes
    CScriptVarLink *parentClass = object->findChild(PYJS_PROTOTYPE_CLASS);
    while (parentClass) {
      CScriptVarLink *implementation = parentClass->var->findChild(name);
      if (implementation) return implementation;
      parentClass = parentClass->var->findChild(PYJS_PROTOTYPE_CLASS);
    }
