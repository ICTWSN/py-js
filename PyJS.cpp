...
void CScriptLex::getNextToken() {
	...
	if (isAlpha(currCh)) { //  IDs
...
#ifdef NO_GENERATORS
		if(tk == LEX_R_YIELD)
			throw new CScriptException(Error, "42PYJS was built without support of generators (yield expression)", currentFile, pos.currentLine, currentColumn());
#endif
		...
		if (tk=='=' && currCh=='=') { // ==
			...
			if(tk == LEX_REGEXP) {
#ifdef NO_REGEXP
				throw new CScriptException(Error, "42PYJS was built without support of regular expressions", currentFile, pos.currentLine, currentColumn());
#endif
				tkStr = "/";
...
CScriptVar::CScriptVar(CPyJS *Context, const CScriptVarPtr &Prototype) {
	extensible = true;
	context = Context;
	...
	refs = 0;
	if(Prototype)
		addChild(PYJS___PROTO___VAR, Prototype, SCRIPTVARLINK_WRITABLE);
#if DEBUG_MEMORY
	mark_allocated(this);
#endif
}
...
CScriptVarPtr CScriptVar::getOwnPropertyDescriptor(const string &Name) {
	...
	if(child->getVarPtr()->isAccessor()) {
		CScriptVarLinkPtr value = child->getVarPtr()->findChild(PYJS_ACCESSOR_GET_VAR);
		ret->addChild("get", value ? value->getVarPtr() : constScriptVar(Undefined));
		value = child->getVarPtr()->findChild(PYJS_ACCESSOR_SET_VAR);
	...
	if(!child) {
		...
		if(!child->isConfigurable()) {
			...
			if(child->getVarPtr()->isAccessor()) {
				...
				if(attr_get && attr_get != child->getVarPtr()->findChild(PYJS_ACCESSOR_GET_VAR)) goto cant_redefine;
				if(attr_set && attr_set != child->getVarPtr()->findChild(PYJS_ACCESSOR_SET_VAR)) goto cant_redefine;
		...
		if(attr_isDataDescriptor) {
			...
			if(child->getVarPtr()->isAccessor()) {
				if(!attr_get) attr_get = child->getVarPtr()->findChild(PYJS_ACCESSOR_GET_VAR);
				if(!attr_set) attr_set = child->getVarPtr()->findChild(PYJS_ACCESSOR_SET_VAR);
			}
...
CScriptVarLinkPtr CScriptVar::findChildInPrototypeChain(const string &childName) {
	...
	while( object->getTemporaryMark() != uniqueID && (__proto__ = object->findChild(PYJS___PROTO___VAR)) ) {
...
CScriptVarLinkPtr CScriptVar::findChildOrCreate(const string &childName/*, int varFlags*/) {
	...
	//	return addChild(childName, new CScriptVar(context, PYJS_BLANK_DATA, varFlags));
}
...
void CScriptVar::keys(set<string> &Keys, bool OnlyEnumerable/*=true*/, uint32_t ID/*=0*/)
{
	...
	CScriptVarLinkPtr __proto__;
	if( ID && (__proto__ = findChild(PYJS___PROTO___VAR)) && __proto__->getVarPtr()->getTemporaryMark() != ID )
...
CScriptVarLink::CScriptVarLink(const CScriptVarPtr &Var, const string &Name /*=PYJS_TEMP_NAME*/, int Flags /*=SCRIPTVARLINK_DEFAULT*/) 
...
CScriptVarLinkPtr & CScriptVarLinkPtr::operator()( const CScriptVarPtr &var, const std::string &name /*= PYJS_TEMP_NAME*/, int flags /*= SCRIPTVARLINK_DEFAULT*/ ) {
...
CScriptVarLinkWorkPtr CScriptVarLinkWorkPtr::getter(CScriptResult &execute) {
	if(execute && link && link->getVarPtr() && link->getVarPtr()->isAccessor()) {
		...
		CScriptVarLinkPtr getter = var->findChild(PYJS_ACCESSOR_GET_VAR);
		if(getter) {
			vector<CScriptVarPtr> Params;
	...
	if(execute) {
		if(link) {
			if(link->getVarPtr() && link->getVarPtr()->isAccessor()) {
				const CScriptVarPtr &var = link->getVarPtr();
				CScriptVarLinkPtr setter = var->findChild(PYJS_ACCESSOR_SET_VAR);
				if(setter) {
					vector<CScriptVarPtr> Params;
...
CScriptVarUndefined::CScriptVarUndefined(CPyJS *Context) : CScriptVarPrimitive(Context, Context->objectPrototype) { }
...
CScriptVarNull::CScriptVarNull(CPyJS *Context) : CScriptVarPrimitive(Context, Context->objectPrototype) { }
...
CScriptVarString::CScriptVarString(CPyJS *Context, const string &Data) : CScriptVarPrimitive(Context, Context->stringPrototype), data(Data) {
...
/*
	...
	acc->getVarPtr()->addChild(PYJS_ACCESSOR_GET_VAR, getter, 0);
*/
}
...
CScriptVarNumber::CScriptVarNumber(CPyJS *Context, const CNumber &Data) : CScriptVarPrimitive(Context, Context->numberPrototype), data(Data) {}
...
inline define_newScriptVar_Fnc(Number, CPyJS *Context, const CNumber &Obj) { 
...
CScriptVarBool::CScriptVarBool(CPyJS *Context, bool Data) : CScriptVarPrimitive(Context, Context->booleanPrototype), data(Data) {}
...
CScriptVarObject::CScriptVarObject(CPyJS *Context) : CScriptVar(Context, Context->objectPrototype) { }
...
CScriptVarError::CScriptVarError(CPyJS *Context, ERROR_TYPES type, const char *message, const char *file, int line, int column) : CScriptVarObject(Context, Context->getErrorPrototype(type)) {
...
CScriptVarArray::CScriptVarArray(CPyJS *Context) : CScriptVarObject(Context, Context->arrayPrototype), toStringRecursion(false) {
	CScriptVarLinkPtr acc = addChild("length", newScriptVar(Accessor), 0);
...
CScriptVarRegExp::CScriptVarRegExp(CPyJS *Context, const string &Regexp, const string &Flags) : CScriptVarObject(Context, Context->regexpPrototype), regexp(Regexp), flags(Flags) {
...
CScriptVarDefaultIterator::CScriptVarDefaultIterator(CPyJS *Context, const CScriptVarPtr &Object, int Mode) 
...
CScriptVarGenerator::CScriptVarGenerator(CPyJS *Context, const CScriptVarPtr &FunctionRoot, const CScriptVarFunctionPtr &Function) 
...
CScriptVarFunction::CScriptVarFunction(CPyJS *Context, CScriptTokenDataFnc *Data) : CScriptVarObject(Context, Context->functionPrototype), data(0) { 
...
CScriptVarAccessor::CScriptVarAccessor(CPyJS *Context) : CScriptVarObject(Context, Context->objectPrototype) { }
CScriptVarAccessor::CScriptVarAccessor(CPyJS *Context, JSCallback getterFnc, void *getterData, JSCallback setterFnc, void *setterData) 
	: CScriptVarObject(Context) 
{
	if(getterFnc)
		addChild(PYJS_ACCESSOR_GET_VAR, ::newScriptVar(Context, getterFnc, getterData), 0);
	if(setterFnc)
		addChild(PYJS_ACCESSOR_SET_VAR, ::newScriptVar(Context, setterFnc, setterData), 0);
}

CScriptVarAccessor::CScriptVarAccessor( CPyJS *Context, const CScriptVarFunctionPtr &getter, const CScriptVarFunctionPtr &setter) : CScriptVarObject(Context, Context->objectPrototype) {
	if(getter)
		addChild(PYJS_ACCESSOR_GET_VAR, getter, 0);
	if(setter)
		addChild(PYJS_ACCESSOR_SET_VAR, setter, 0);
}
...
void CScriptVarScopeFnc::setReturnVar(const CScriptVarPtr &var) {
	addChildOrReplace(PYJS_RETURN_VAR, var);
}
...
CScriptVarPtr CScriptVarScopeFnc::getArgument(int Idx) {
	CScriptVarLinkPtr arguments = findChildOrCreate(PYJS_ARGUMENTS_VAR);
...
int CScriptVarScopeFnc::getArgumentsLength() {
	CScriptVarLinkPtr arguments = findChild(PYJS_ARGUMENTS_VAR);
...
CScriptVarScopeLet::CScriptVarScopeLet(const CScriptVarScopePtr &Parent) // constructor for LetScope
	: CScriptVarScope(Parent->getContext()), parent(addChild(PYJS_SCOPE_PARENT_VAR, Parent, 0))
...
extern "C" void _registerFunctions(CPyJS *PYJS);
extern "C" void _registerStringFunctions(CPyJS *PYJS);
extern "C" void _registerMathFunctions(CPyJS *PYJS);
...
CPyJS::CPyJS() {
	...
	objectPrototype->addChild(PYJS___PROTO___VAR, objectPrototype, 0);
	...
	//////////////////////////////////////////////////////////////////////////
	// Object
	var = addNative("function Object()", this, &CPyJS::native_Object, 0, SCRIPTVARLINK_CONSTANT);
	objectPrototype = var->findChild(PYJS_PROTOTYPE_CLASS);
	objectPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	addNative("function Object.getPrototypeOf(obj)", this, &CPyJS::native_Object_getPrototypeOf); 
	addNative("function Object.preventExtensions(obj)", this, &CPyJS::native_Object_setObjectSecure); 
	addNative("function Object.isExtensible(obj)", this, &CPyJS::native_Object_isSecureObject); 
	addNative("function Object.seel(obj)", this, &CPyJS::native_Object_setObjectSecure, (void*)1); 
	addNative("function Object.isSealed(obj)", this, &CPyJS::native_Object_isSecureObject, (void*)1); 
	addNative("function Object.freeze(obj)", this, &CPyJS::native_Object_setObjectSecure, (void*)2); 
	addNative("function Object.isFrozen(obj)", this, &CPyJS::native_Object_isSecureObject, (void*)2); 
	addNative("function Object.keys(obj)", this, &CPyJS::native_Object_keys); 
	addNative("function Object.getOwnPropertyNames(obj)", this, &CPyJS::native_Object_keys, (void*)1); 
	addNative("function Object.getOwnPropertyDescriptor(obj,name)", this, &CPyJS::native_Object_getOwnPropertyDescriptor); 
	addNative("function Object.defineProperty(obj,name,attributes)", this, &CPyJS::native_Object_defineProperty); 
	addNative("function Object.defineProperties(obj,properties)", this, &CPyJS::native_Object_defineProperties); 
	addNative("function Object.create(obj,properties)", this, &CPyJS::native_Object_defineProperties, (void*)1); 

	addNative("function Object.prototype.hasOwnProperty(prop)", this, &CPyJS::native_Object_prototype_hasOwnProperty); 
	objectPrototype_valueOf = addNative("function Object.prototype.valueOf()", this, &CPyJS::native_Object_prototype_valueOf); 
	objectPrototype_toString = addNative("function Object.prototype.toString(radix)", this, &CPyJS::native_Object_prototype_toString); 
	...
	//////////////////////////////////////////////////////////////////////////
	// Array
	var = addNative("function Array()", this, &CPyJS::native_Array, 0, SCRIPTVARLINK_CONSTANT);
	arrayPrototype = var->findChild(PYJS_PROTOTYPE_CLASS);
	arrayPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	...
	var = addNative("function Array.__constructor__()", this, &CPyJS::native_Array, (void*)1, SCRIPTVARLINK_CONSTANT);
	var->getFunctionData()->name = "Array";

	//////////////////////////////////////////////////////////////////////////
	// String
	var = addNative("function String()", this, &CPyJS::native_String, 0, SCRIPTVARLINK_CONSTANT);
	stringPrototype  = var->findChild(PYJS_PROTOTYPE_CLASS);
	stringPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	...
	var = addNative("function String.__constructor__()", this, &CPyJS::native_String, (void*)1, SCRIPTVARLINK_CONSTANT);
	var->getFunctionData()->name = "String";

	//////////////////////////////////////////////////////////////////////////
	// RegExp
#ifndef NO_REGEXP
	var = addNative("function RegExp()", this, &CPyJS::native_RegExp, 0, SCRIPTVARLINK_CONSTANT);
	regexpPrototype  = var->findChild(PYJS_PROTOTYPE_CLASS);
	regexpPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	...
	//////////////////////////////////////////////////////////////////////////
	// Number
	var = addNative("function Number()", this, &CPyJS::native_Number, 0, SCRIPTVARLINK_CONSTANT);
	...
	numberPrototype = var->findChild(PYJS_PROTOTYPE_CLASS);
	numberPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	...
	var = addNative("function Number.__constructor__()", this, &CPyJS::native_Number, (void*)1, SCRIPTVARLINK_CONSTANT);
	var->getFunctionData()->name = "Number";

	//////////////////////////////////////////////////////////////////////////
	// Boolean
	var = addNative("function Boolean()", this, &CPyJS::native_Boolean, 0, SCRIPTVARLINK_CONSTANT);
	booleanPrototype = var->findChild(PYJS_PROTOTYPE_CLASS);
	booleanPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	...
	var = addNative("function Boolean.__constructor__()", this, &CPyJS::native_Boolean, (void*)1, SCRIPTVARLINK_CONSTANT);
	var->getFunctionData()->name = "Boolean";

	//////////////////////////////////////////////////////////////////////////
	// Iterator
	var = addNative("function Iterator(obj,mode)", this, &CPyJS::native_Iterator, 0, SCRIPTVARLINK_CONSTANT); 
	iteratorPrototype = var->findChild(PYJS_PROTOTYPE_CLASS);
	iteratorPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
...
//	var = addNative("function Iterator(obj,mode)", this, &CPyJS::native_Iterator, 0, SCRIPTVARLINK_CONSTANT); 
	...
	generatorPrototype->addChild("next", ::newScriptVar(this, this, &CPyJS::native_Generator_prototype_next, (void*)0, "Generator.next"), SCRIPTVARLINK_BUILDINDEFAULT);
	generatorPrototype->addChild("send", ::newScriptVar(this, this, &CPyJS::native_Generator_prototype_next, (void*)1, "Generator.send"), SCRIPTVARLINK_BUILDINDEFAULT);
	generatorPrototype->addChild("close", ::newScriptVar(this, this, &CPyJS::native_Generator_prototype_next, (void*)2, "Generator.close"), SCRIPTVARLINK_BUILDINDEFAULT);
	generatorPrototype->addChild("throw", ::newScriptVar(this, this, &CPyJS::native_Generator_prototype_next, (void*)3, "Generator.throw"), SCRIPTVARLINK_BUILDINDEFAULT);
	pseudo_refered.push_back(&generatorPrototype);
#endif /*NO_GENERATORS*/

	//////////////////////////////////////////////////////////////////////////
	// Function
	var = addNative("function Function(params, body)", this, &CPyJS::native_Function, 0, SCRIPTVARLINK_CONSTANT); 
	var->addChildOrReplace(PYJS_PROTOTYPE_CLASS, functionPrototype);
	functionPrototype->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	addNative("function Function.prototype.call(objc)", this, &CPyJS::native_Function_prototype_call); 
	addNative("function Function.prototype.apply(objc, args)", this, &CPyJS::native_Function_prototype_apply); 
	addNative("function Function.prototype.bind(objc, args)", this, &CPyJS::native_Function_prototype_bind); 
	...
	//////////////////////////////////////////////////////////////////////////
	// Error
	var = addNative("function Error(message, fileName, lineNumber, column)", this, &CPyJS::native_Error, 0, SCRIPTVARLINK_CONSTANT); 
	errorPrototypes[Error] = var->findChild(PYJS_PROTOTYPE_CLASS);
	errorPrototypes[Error]->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	...
	var = addNative("function EvalError(message, fileName, lineNumber, column)", this, &CPyJS::native_EvalError, 0, SCRIPTVARLINK_CONSTANT); 
	errorPrototypes[EvalError] = var->findChild(PYJS_PROTOTYPE_CLASS);
	errorPrototypes[EvalError]->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	errorPrototypes[EvalError]->addChildOrReplace(PYJS___PROTO___VAR, errorPrototypes[Error], SCRIPTVARLINK_WRITABLE);
	errorPrototypes[EvalError]->addChild("name", newScriptVar("EvalError"));

	var = addNative("function RangeError(message, fileName, lineNumber, column)", this, &CPyJS::native_RangeError, 0, SCRIPTVARLINK_CONSTANT); 
	errorPrototypes[RangeError] = var->findChild(PYJS_PROTOTYPE_CLASS);
	errorPrototypes[RangeError]->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	errorPrototypes[RangeError]->addChildOrReplace(PYJS___PROTO___VAR, errorPrototypes[Error], SCRIPTVARLINK_WRITABLE);
	errorPrototypes[RangeError]->addChild("name", newScriptVar("RangeError"));

	var = addNative("function ReferenceError(message, fileName, lineNumber, column)", this, &CPyJS::native_ReferenceError, 0, SCRIPTVARLINK_CONSTANT); 
	errorPrototypes[ReferenceError] = var->findChild(PYJS_PROTOTYPE_CLASS);
	errorPrototypes[ReferenceError]->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	errorPrototypes[ReferenceError]->addChildOrReplace(PYJS___PROTO___VAR, errorPrototypes[Error], SCRIPTVARLINK_WRITABLE);
	errorPrototypes[ReferenceError]->addChild("name", newScriptVar("ReferenceError"));

	var = addNative("function SyntaxError(message, fileName, lineNumber, column)", this, &CPyJS::native_SyntaxError, 0, SCRIPTVARLINK_CONSTANT); 
	errorPrototypes[SyntaxError] = var->findChild(PYJS_PROTOTYPE_CLASS);
	errorPrototypes[SyntaxError]->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	errorPrototypes[SyntaxError]->addChildOrReplace(PYJS___PROTO___VAR, errorPrototypes[Error], SCRIPTVARLINK_WRITABLE);
	errorPrototypes[SyntaxError]->addChild("name", newScriptVar("SyntaxError"));

	var = addNative("function TypeError(message, fileName, lineNumber, column)", this, &CPyJS::native_TypeError, 0, SCRIPTVARLINK_CONSTANT); 
	errorPrototypes[TypeError] = var->findChild(PYJS_PROTOTYPE_CLASS);
	errorPrototypes[TypeError]->addChild(PYJS_CONSTRUCTOR_VAR, var, SCRIPTVARLINK_BUILDINDEFAULT);
	errorPrototypes[TypeError]->addChildOrReplace(PYJS___PROTO___VAR, errorPrototypes[Error], SCRIPTVARLINK_WRITABLE);
	...
	constStopIteration->addChild(PYJS_PROTOTYPE_CLASS, var, SCRIPTVARLINK_CONSTANT);	pseudo_refered.push_back(&constStopIteration);
	...
	addNative("function eval(jsCode)", this, &CPyJS::native_eval);
	native_require_read = 0;
	addNative("function require(jsFile)", this, &CPyJS::native_require);
	addNative("function isNaN(objc)", this, &CPyJS::native_isNAN);
	addNative("function isFinite(objc)", this, &CPyJS::native_isFinite);
	addNative("function parseInt(string, radix)", this, &CPyJS::native_parseInt);
	addNative("function parseFloat(string)", this, &CPyJS::native_parseFloat);
	
	root->addChild("JSON", newScriptVar(Object), SCRIPTVARLINK_BUILDINDEFAULT);
	addNative("function JSON.parse(text, reviver)", this, &CPyJS::native_JSON_parse);
...
void CPyJS::throwError(CScriptResult &execute, ERROR_TYPES ErrorType, const string &message ) {
	if(execute && haveTry) {
...
void CPyJS::throwException(ERROR_TYPES ErrorType, const string &message ) {
	throw new CScriptException(ErrorType, message, t->currentFile, t->currentLine(), t->currentColumn());
}

void CPyJS::throwError(CScriptResult &execute, ERROR_TYPES ErrorType, const string &message, CScriptTokenizer::ScriptTokenPosition &Pos ){
	if(execute && haveTry) {
...
void CPyJS::throwException(ERROR_TYPES ErrorType, const string &message, CScriptTokenizer::ScriptTokenPosition &Pos ){
	throw new CScriptException(ErrorType, message, t->currentFile, Pos.currentLine(), Pos.currentColumn());
}
...
void CPyJS::execute(CScriptTokenizer &Tokenizer) {
	evaluateComplex(Tokenizer);
}

void CPyJS::execute(const char *Code, const string &File, int Line, int Column) {
	evaluateComplex(Code, File, Line, Column);
}

void CPyJS::execute(const string &Code, const string &File, int Line, int Column) {
	evaluateComplex(Code, File, Line, Column);
}

CScriptVarLinkPtr CPyJS::evaluateComplex(CScriptTokenizer &Tokenizer) {
	t = &Tokenizer;
	CScriptResult execute;
	try {
		do {
...
CScriptVarLinkPtr CPyJS::evaluateComplex(const char *Code, const string &File, int Line, int Column) {
	CScriptTokenizer Tokenizer(Code, File, Line, Column);
	return evaluateComplex(Tokenizer);
}
CScriptVarLinkPtr CPyJS::evaluateComplex(const string &Code, const string &File, int Line, int Column) {
	CScriptTokenizer Tokenizer(Code.c_str(), File, Line, Column);
	return evaluateComplex(Tokenizer);
}

string CPyJS::evaluate(CScriptTokenizer &Tokenizer) {
	return evaluateComplex(Tokenizer)->toString();
}
string CPyJS::evaluate(const char *Code, const string &File, int Line, int Column) {
	return evaluateComplex(Code, File, Line, Column)->toString();
}
string CPyJS::evaluate(const string &Code, const string &File, int Line, int Column) {
	return evaluate(Code.c_str(), File, Line, Column);
}

CScriptVarFunctionNativePtr CPyJS::addNative(const string &funcDesc, JSCallback ptr, void *userdata, int LinkFlags) {
	return addNative(funcDesc, ::newScriptVar(this, ptr, userdata), LinkFlags);
}

CScriptVarFunctionNativePtr CPyJS::addNative(const string &funcDesc, CScriptVarFunctionNativePtr Var, int LinkFlags) {
	...
	Var->addChild(PYJS_PROTOTYPE_CLASS, newScriptVar(Object), SCRIPTVARLINK_WRITABLE);
...
CScriptVarLinkWorkPtr CPyJS::parseFunctionDefinition(const CScriptToken &FncToken) {
	const CScriptTokenDataFnc &Fnc = FncToken.Fnc();
//	string fncName = (FncToken.token == LEX_T_FUNCTION_OPERATOR) ? PYJS_TEMP_NAME : Fnc.name;
	CScriptVarLinkWorkPtr funcVar(newScriptVar((CScriptTokenDataFnc*)&Fnc), Fnc.name);
	if(scope() != root)
		funcVar->getVarPtr()->addChild(PYJS_FUNCTION_CLOSURE_VAR, scope(), 0);
	funcVar->getVarPtr()->addChild(PYJS_PROTOTYPE_CLASS, newScriptVar(Object), SCRIPTVARLINK_WRITABLE)->getVarPtr()->addChild(PYJS_CONSTRUCTOR_VAR, funcVar->getVarPtr(), SCRIPTVARLINK_WRITABLE);
	return funcVar;
}

CScriptVarLinkWorkPtr CPyJS::parseFunctionsBodyFromString(const string &ArgumentList, const string &FncBody) {
...
CScriptVarPtr CPyJS::callFunction(const CScriptVarFunctionPtr &Function, vector<CScriptVarPtr> &Arguments, const CScriptVarPtr &This, CScriptVarPtr *newThis) {
	CScriptResult execute;
...
CScriptVarPtr CPyJS::callFunction(CScriptResult &execute, const CScriptVarFunctionPtr &Function, vector<CScriptVarPtr> &Arguments, const CScriptVarPtr &This, CScriptVarPtr *newThis) {
	...
	CScriptVarScopeFncPtr functionRoot(::newScriptVar(this, ScopeFnc, CScriptVarPtr(Function->findChild(PYJS_FUNCTION_CLOSURE_VAR))));
	if(Fnc->name.size()) functionRoot->addChild(Fnc->name, Function);
	functionRoot->addChild("this", This);
	CScriptVarPtr arguments = functionRoot->addChild(PYJS_ARGUMENTS_VAR, newScriptVar(Object));
	...
	if (Function->isNative()) {
		try {
			...
			CScriptVarLinkPtr ret = functionRoot->findChild(PYJS_RETURN_VAR);
...
#ifndef NO_GENERATORS
void CPyJS::generator_start(CScriptVarGenerator *Generator)
{
...
CScriptVarPtr CPyJS::generator_yield(CScriptResult &execute, CScriptVar *YieldIn)
{
...
CScriptVarPtr CPyJS::mathsOp(CScriptResult &execute, const CScriptVarPtr &A, const CScriptVarPtr &B, int op) {
	if(!execute) return constUndefined;
...
void CPyJS::assign_destructuring_var(const CScriptVarPtr &Scope, const CScriptTokenDataDestructuringVar &Objc, const CScriptVarPtr &Val, CScriptResult &execute) {
	if(!execute) return;
	if(Objc.vars.size() == 1) {
		if(Scope)
...
void CPyJS::execute_var_init( bool hideLetScope, CScriptResult &execute )
{
	for(;;) {
...
void CPyJS::execute_destructuring(CScriptTokenDataObjectLiteral &Objc, const CScriptVarPtr &Val, CScriptResult &execute) {
...
CScriptVarLinkWorkPtr CPyJS::execute_literals(CScriptResult &execute) {
	switch(t->tk) {
	case LEX_ID: 
		if(execute) {
				...
				for(vector<CScriptTokenDataObjectLiteral::ELEMENT>::iterator it=Objc.elements.begin(); execute && it!=Objc.elements.end(); ++it) {
					...
					if(tk.token==LEX_T_GET || tk.token==LEX_T_SET) {
						...
						if((tk.token == LEX_T_GET && Fnc.arguments.size()==0) || (tk.token == LEX_T_SET && Fnc.arguments.size()==1)) {
							...
							child->getVarPtr()->addChildOrReplace((tk.token==LEX_T_GET?PYJS_ACCESSOR_GET_VAR:PYJS_ACCESSOR_SET_VAR), funcVar->getVarPtr());
						}
					} else {
						t->pushTokenScope(it->value);
		...
		if (execute) {
			...
			if (execute) {
				...
				if(Constructor->isFunction()) {
					...
					CScriptVarLinkPtr prototype = Constructor->findChild(PYJS_PROTOTYPE_CLASS);
					if(!prototype || prototype->getVarPtr()->isUndefined() || prototype->getVarPtr()->isNull()) {
						prototype = Constructor->addChild(PYJS_PROTOTYPE_CLASS, newScriptVar(Object), SCRIPTVARLINK_WRITABLE);
						obj->addChildOrReplace(PYJS___PROTO___VAR, prototype, SCRIPTVARLINK_WRITABLE);
					}
...
CScriptVarLinkWorkPtr CPyJS::execute_member(CScriptVarLinkWorkPtr &parent, CScriptResult &execute) {
...
CScriptVarLinkWorkPtr CPyJS::execute_function_call(CScriptResult &execute) {
	CScriptVarLinkWorkPtr parent = execute_literals(execute);
...
bool CPyJS::execute_unary_rhs(CScriptResult &execute, CScriptVarLinkWorkPtr& a) {
	t->match(t->tk);
	a = execute_unary(execute).getter(execute);
	if(execute) CheckRightHandVar(execute, a);
	return execute;
};
CScriptVarLinkWorkPtr CPyJS::execute_unary(CScriptResult &execute) {
	CScriptVarLinkWorkPtr a;
	switch(t->tk) {
	case '-':
...
CScriptVarLinkWorkPtr CPyJS::execute_term(CScriptResult &execute) {
...
CScriptVarLinkWorkPtr CPyJS::execute_expression(CScriptResult &execute) {
...
CScriptVarLinkWorkPtr CPyJS::execute_binary_shift(CScriptResult &execute) {
...
CScriptVarLinkWorkPtr CPyJS::execute_relation(CScriptResult &execute, int set, int set_n) {
		...
		while ((set==LEX_EQUAL && t->tk>=LEX_RELATIONS_1_BEGIN && t->tk<=LEX_RELATIONS_1_END)
			...
			if (execute) {
				...
				if(op == LEX_R_IN) {
					...
					CScriptVarLinkPtr prototype = b->getVarPtr()->findChild(PYJS_PROTOTYPE_CLASS);
					if(!prototype)
						throwError(execute, TypeError, "invalid 'instanceof' operand "+nameOf_b);
					else {
						unsigned int uniqueID = allocUniqueID();
						CScriptVarPtr object = a->getVarPtr()->findChild(PYJS___PROTO___VAR);
						while( object && object!=prototype->getVarPtr() && object->getTemporaryMark() != uniqueID) {
							object->setTemporaryMark(uniqueID); // prevents recursions
							object = object->findChild(PYJS___PROTO___VAR);
						}
						freeUniqueID();
...
CScriptVarLinkWorkPtr CPyJS::execute_binary_logic(CScriptResult &execute, int op, int op_n1, int op_n2) {
...
CScriptVarLinkWorkPtr CPyJS::execute_logic(CScriptResult &execute, int op /*= LEX_OROR*/, int op_n /*= LEX_ANDAND*/) {
...
CScriptVarLinkWorkPtr CPyJS::execute_condition(CScriptResult &execute) {
...
CScriptVarLinkPtr CPyJS::execute_assignment(CScriptResult &execute) {
	return execute_assignment(execute_condition(execute), execute);
}
CScriptVarLinkPtr CPyJS::execute_assignment(CScriptVarLinkWorkPtr lhs, CScriptResult &execute) {
...
CScriptVarLinkPtr CPyJS::execute_base(CScriptResult &execute) {
	CScriptVarLinkPtr a;
	for(;;)
	{
...
void CPyJS::execute_block(CScriptResult &execute) {
	if(execute) {
		t->match('{');
		CScopeControl ScopeControl(this);
...
void CPyJS::execute_statement(CScriptResult &execute) {
	switch(t->tk) {
...
CScriptVarLinkPtr CPyJS::findInScopes(const string &childName) {
...
void CPyJS::native_Object(const CFunctionsScopePtr &c, void *data) {
	c->setReturnVar(c->getArgument(0)->toObject());
}
void CPyJS::native_Object_getPrototypeOf(const CFunctionsScopePtr &c, void *data) {
	if(c->getArgumentsLength()>=1) {
		CScriptVarPtr obj = c->getArgument(0);
		if(obj->isObject()) {
			c->setReturnVar(obj->findChild(PYJS___PROTO___VAR));
			return;
		}
	}
...
void CPyJS::native_Object_setObjectSecure(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr obj = c->getArgument(0);
...
void CPyJS::native_Object_isSecureObject(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr obj = c->getArgument(0);
	if(!obj->isObject()) c->throwError(TypeError, "argument is not an object");
	bool ret;
	if(data==(void*)2)
...
void CPyJS::native_Object_keys(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr obj = c->getArgument(0);
	if(!obj->isObject()) c->throwError(TypeError, "argument is not an object");
...
void CPyJS::native_Object_getOwnPropertyDescriptor(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr obj = c->getArgument(0);
	if(!obj->isObject()) c->throwError(TypeError, "argument is not an object");
	c->setReturnVar(obj->getOwnPropertyDescriptor(c->getArgument(1)->toString()));
}

void CPyJS::native_Object_defineProperty(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr obj = c->getArgument(0);
	if(!obj->isObject()) c->throwError(TypeError, "argument is not an object");
...
void CPyJS::native_Object_defineProperties(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::native_Object_prototype_hasOwnProperty(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::native_Object_prototype_valueOf(const CFunctionsScopePtr &c, void *data) {
	c->setReturnVar(c->getArgument("this")->valueOf_CallBack());
}
void CPyJS::native_Object_prototype_toString(const CFunctionsScopePtr &c, void *data) {
	CScriptResult execute;
	int radix = 10;
...
void CPyJS::native_Array(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr returnVar = c->newScriptVar(Array);
	c->setReturnVar(returnVar);
...
void CPyJS::native_String(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr arg;
...
void CPyJS::native_RegExp(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::native_Number(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr arg;
	if(c->getArgumentsLength()==0)
		arg = newScriptVar(0);
	else
...
void CPyJS::native_Boolean(const CFunctionsScopePtr &c, void *data) {
	CScriptVarPtr arg;
	if(c->getArgumentsLength()==0)
...
void CPyJS::native_Iterator(const CFunctionsScopePtr &c, void *data) {
...
#ifndef NO_GENERATORS
void CPyJS::native_Generator_prototype_next(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::native_Function(const CFunctionsScopePtr &c, void *data) {
	int length = c->getArgumentsLength();
	string params, body;
	if(length>=1) 
...
void CPyJS::native_Function_prototype_call(const CFunctionsScopePtr &c, void *data) {
	int length = c->getArgumentsLength();
...
void CPyJS::native_Function_prototype_apply(const CFunctionsScopePtr &c, void *data) {
	int length=0;
	CScriptVarPtr Fnc = c->getArgument("this");
...
void CPyJS::native_Function_prototype_bind(const CFunctionsScopePtr &c, void *data) {
	int length = c->getArgumentsLength();
	CScriptVarPtr Fnc = c->getArgument("this");
...
static CScriptVarPtr _newError(CPyJS *context, ERROR_TYPES type, const CFunctionsScopePtr &c) {
...
void CPyJS::native_Error(const CFunctionsScopePtr &c, void *data) { c->setReturnVar(_newError(this, Error,c)); }
void CPyJS::native_EvalError(const CFunctionsScopePtr &c, void *data) { c->setReturnVar(_newError(this, EvalError,c)); }
void CPyJS::native_RangeError(const CFunctionsScopePtr &c, void *data) { c->setReturnVar(_newError(this, RangeError,c)); }
void CPyJS::native_ReferenceError(const CFunctionsScopePtr &c, void *data){ c->setReturnVar(_newError(this, ReferenceError,c)); }
void CPyJS::native_SyntaxError(const CFunctionsScopePtr &c, void *data){ c->setReturnVar(_newError(this, SyntaxError,c)); }
void CPyJS::native_TypeError(const CFunctionsScopePtr &c, void *data){ c->setReturnVar(_newError(this, TypeError,c)); }

////////////////////////////////////////////////////////////////////////// 
/// global functions
//////////////////////////////////////////////////////////////////////////

void CPyJS::native_eval(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::native_require(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::native_JSON_parse(const CFunctionsScopePtr &c, void *data) {
...
void CPyJS::setTemporaryID_recursive(uint32_t ID) {
	for(vector<CScriptVarPtr*>::iterator it = pseudo_refered.begin(); it!=pseudo_refered.end(); ++it)
...
void CPyJS::ClearUnreferedVars(const CScriptVarPtr &extra/*=CScriptVarPtr()*/) {
	uint32_t UniqueID = allocUniqueID(); 
	setTemporaryID_recursive(UniqueID);
